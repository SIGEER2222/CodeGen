...
public class SourceFile {
  ...
  public SourceFile(string path) {
    this.SyntaxTree = CSharpSyntaxTree.ParseText(File.ReadAllText(path));
    this.EntryPath = path;
  }
  ...
  private IEnumerable<FileScopedNamespaceDeclarationSyntax> GetNamespacesOfFileScoped() {
    return SyntaxTree.GetRoot().ChildNodes().OfType<FileScopedNamespaceDeclarationSyntax>();
  }

  public IEnumerable<string> Usings {
    get {
      return SyntaxTree.GetRoot().ChildNodes()
          .OfType<UsingDirectiveSyntax>()
          .Where(x => x is { Name: { } })
          .Where(x => x.StaticKeyword == default)
  ...
  public IEnumerable<string> Namespaces {
    get {
      var namespaces = GetNamespaces().Select(x => x.Name.ToString());
      var fileScopedNamespaces = GetNamespacesOfFileScoped().Select(x => x.Name.ToString());
      return namespaces.Concat(fileScopedNamespaces);
    }
  }
  ...
  public IEnumerable<TypeDeclaration> GetAllPartialClasses() {
    return SyntaxTree.GetRoot().DescendantNodes()
        .OfType<ClassDeclarationSyntax>()
        .Where(x => x.Modifiers.Any(m => m.IsKind(SyntaxKind.PartialKeyword)))
        .Select(y => new TypeDeclaration(this, y));
  }
  ...
  public string GeneratedPath => SyntaxTree.FilePath;
  public string FilePath => "./" + SyntaxTree.FilePath.Substring(EntryPath.Length + 1).Replace('\\', '/');
