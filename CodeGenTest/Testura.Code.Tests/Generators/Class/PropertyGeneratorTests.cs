...
public class PropertyGeneratorTests {
  ...
  public void Create_WhenCreatingAutoPropertyWithAttribute_ShouldHaveAttribute() {
    Assert.AreEqual("[Test]intMyProperty{get;set;}", PropertyGenerator.Create(new AutoProperty("MyProperty", typeof(int), PropertyTypes.GetAndSet, new List<Modifiers>(), new List<TesturaAttribute> { new TesturaAttribute("Test") })).ToString());
  }
  ...
  public void Create_WhenCreatingBodyPropertyWithAttribute_ShouldHaveAttribute() {
    Assert.AreEqual("[Test]intMyProperty{get{return1;}}", PropertyGenerator.Create(new BodyProperty(
        ...
        attributes: new List<TesturaAttribute> { new TesturaAttribute("Test") })).ToString());
  }
