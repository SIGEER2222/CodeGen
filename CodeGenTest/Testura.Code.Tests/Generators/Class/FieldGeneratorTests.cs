...
public class FieldGeneratorTests {
  ...
  public void Create_WhenCreatingFieldWithAttribute_ShouldGenerateCorrectCode() {
    Assert.AreEqual("[Test]intmyField;", FieldGenerator.Create(new Field("myField", typeof(int), attributes: new List<TesturaAttribute>() { new TesturaAttribute("Test") })).ToString());
  }
}
